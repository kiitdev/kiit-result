 ...
 */
const result_1 = require("@kiit/result");
const { Success, Failure, Action, withAction } = result_1.kiit.result;
...
// result.flatMap(fn).
const mapped = result_1.kiit.result.flatMap(bare, (v) => Success.of(v + 1));
if (mapped instanceof Success) {
    console.log("mapped:", mapped.value);
}
const orElse = result_1.kiit.result.getOrElse(failBare, () => -1);
...
const tried = result_1.kiit.result.Result.attempt(() => 5);
